...
def train(
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        print(
            f"Epoch {epoch+1:03} | Train Loss: {train_loss:.5f} | Val Loss: {val_loss:.5f} | Train Acc: {train_acc:.2f} | Val Acc: {val_acc:.2f}"
        )
...
def evaluate_accuracy(logits, y_true):
    ...
    correct_pred = (
        y_pred == y_true
    ).float()  # PyCharm error Unresolved attribute reference 'float' for class 'bool'
